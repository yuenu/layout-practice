...
const hamburgerMenuButton = document.querySelector("mobile-nav__button");
...
function onScroll(scroll_pos) {
	...
	if (scroll_pos > 5) {
		...
		if (mobileCheck()) {
			headerEl.style.padding = "10px 5vw";
		}
	} else {
		...
		if (mobileCheck()) {
			headerEl.style.padding = "30px 5vw";
		}
	}
}
...
arrowEl.addEventListener("click", () => {
	...
	let headerOffset = 101;
	if (mobileCheck()) headerOffset = 47;
...
hamburgerMenu.addEventListener("click", (e) => {
	if (hamburgerMenu.children.namedItem("menu-container") === e.target) {
		return;
	}
...
window.addEventListener("click", (e) => {
	if (
		...
		!hamburgerMenu.querySelector("." + e.target.classList)
	) {
		hamburgerMenu.classList.remove("active");
	}
});
